...
```bash
sam-machine-learning$ sam build
```
...
```bash
sam-machine-learning$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-machine-learning$ sam local start-api
sam-machine-learning$ curl http://localhost:3000/classify_digit
```
...
```bash
sam-machine-learning$ sam logs -n InferenceFunction --stack-name "sam-machine-learning" --tail
```
...
```bash
sam delete --stack-name "sam-machine-learning"
```
